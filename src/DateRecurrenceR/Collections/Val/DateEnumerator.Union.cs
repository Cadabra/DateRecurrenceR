...
public partial struct UnionEnumeratorVal : IEnumerator<DateOnly>
{
    private readonly EWrapper[] _enumerators;
    private bool _inited = false;
    private DateOnly? _current = null;
    
    internal UnionEnumeratorVal(IReadOnlyList<IEnumerator<DateOnly>> enumerators)
    {
        var hash = new HashSet<EWrapper>();

        for (var i = 0; i < enumerators.Count; i++)
        {
            if (enumerators[i] is UnionEnumeratorVal ue)
            {
